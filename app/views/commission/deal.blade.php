...
<div data-role="page" class="commmission_deal">
  ...
  {{ Form::open(array('url' => "commission/$dealrecord->id/save",'data-ajax'=>'true')) }}
    ...
    <ul data-role="listview" data-inset="true">
		...
		<li><input type="text"  name="commissionSet[-][comdate_at]" value="{{$commission->comdate_at}}"  placeholder="日期"></li>
		<li>
			<button class="btn_delete ui-btn  ui-shadow  ui-corner-all" >删除</button>
		</li>
    </ul>
    @endforeach
    </div>
	...
	$(function(){
		...
		page.find(".btn_add").click(function(){
			...
			$('<li><input type="text"  name="commissionSet[-][comdate_at]" value=""  placeholder="日期"></li>').appendTo(ul);
			$('<li><button class="btn_delete ui-btn  ui-shadow  ui-corner-all" >删除</button></li>').appendTo(ul);
